...
class SGE(PBS):
    ...
    def check_status(self, job):
        ...
        err_str = stderr.read().decode("utf-8")
        if ret != 0:
            raise RuntimeError("status command qstat fails to execute. erro info: %s return code %d" % (err_str, ret))
        ...
        if status_line is None:
            ...
            while count <= 6:
                if self.check_finish_tag(job=job):
                    return JobStatus.finished
                dlog.info("not tag_finished detected, execute sync command and wait. count " + str(count))
        ...
        return [
            Argument(
                ...
                [
                    ...
                    Argument("job_name", str, optional=True, default="wDPjob", doc=doc_job_name),
                ],
                optional=False,
                doc="Extra arguments.",
            )
