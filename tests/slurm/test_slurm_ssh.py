...
__package__ = "slurm"
from .context import (JobStatus, LocalContext, LocalSession, Slurm,
...
class TestSlurm(unittest.TestCase):
    ...
    def test_gen_sub_script(self):
        ...
        with open("run.sub", "w") as fp:
            fp.write(ret)
        ret1 = self.slurm.sub_script(
            job_dirs, ["touch", "touch"], [["test1 ", "test1 "], ["test2 ", "test2 "]]
        )
    ...
    def test_sub_success(self):
        ...
        self.assertTrue(
            os.path.isfile(
                os.path.join(
                    "rmt",
                    self.slurm.context.remote_root,
                    "%s_tag_finished" % self.slurm.context.job_uuid,
                )
            )
        )
        self.assertTrue(
