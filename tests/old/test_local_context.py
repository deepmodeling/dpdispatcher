...
class TestLocalContext(unittest.TestCase):
    ...
    def test_upload(self):
        ...
        for ii in tasks:
            for jj in ["dir0"]:
                locf = os.path.join("loc", ii, jj)
                rmtf = os.path.join("rmt", self.job.job_uuid, ii, jj)
                self.assertEqual(os.path.realpath(locf), os.path.realpath(rmtf))
    ...
    def test_dl_t_t(self):
        ...
        for ii in tasks:
            for jj in ["dir1"]:
                os.makedirs(
                    os.path.join("rmt", self.job.job_uuid, ii, jj), exist_ok=False
                )
                for kk in ["test6", "test7"]:
                    with open(
                        os.path.join("rmt", self.job.job_uuid, ii, jj, kk), "w"
                    ) as fp:
                        tmp = str(uuid.uuid4())
                        fp.write(tmp)
                        record_uuid.append(tmp)
        ...
        for ii in tasks:
            for jj in ["test7", "test8"]:
                if (ii == "task1" and jj == "test7") or (
                    ...
                    self.assertFalse(
                        os.path.exists(os.path.join("loc", ii, jj)),
